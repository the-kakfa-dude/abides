...
docker rm $(docker ps -a | grep 'ts_http_db_sonar:latest' | awk '{print $NF}')

docker rmi $(docker images | tr -s ' ' | grep 'ts_http_db_sonar latest' | cut -d' ' -f3)

docker image prune --force

docker volume rm $(docker volume ls | tr -s ' ' | grep '^local postgres_ts_http_db_sonar' | cut -d' ' -f2)
